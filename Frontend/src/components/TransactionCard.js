...
const TransactionCard = ({ expense }) => {
  ...
  const handleEditSubmit = (e) => {
    ...
    editExpenses(
      editExpense.id,
      editExpense.eaccount,
      editExpense.eamount,
      editExpense.etitle,
      editExpense.ecategory,
      editExpense.edate,
      editExpense.edescription,
      editExpense.etype
    );

    const modalElement = document.getElementById(`modal-${editExpense.id}`);
    const bootstrapModal = Modal.getInstance(modalElement);
    bootstrapModal.hide();
  };


  const val = expense.type;
  const amountClass = val === "cr" ? "green" : "red";
  const image = expense.category.toLowerCase();
